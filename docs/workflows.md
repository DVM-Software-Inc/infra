...
**Caller setup preconditions (per repo):** (1) create the `prod` GitHub environment **with required reviewers** — `deploy.yml`'s `environment:` line only gates approval if the environment is protected; GitHub auto-creates it unprotected on first use. (2) Pre-provision `deploy_path` on the VPS with its `.env` file before the first deploy — the workflow ships only the compose file. (3) Compose files must live under `deploy/` in the repo (enforced by a validation step; required by the scp `strip_components` contract so the file lands flat in `deploy_path` next to `.env`). (4) Set a unique top-level `name:` in every compose file (e.g. `<slug>-dev`) — compose otherwise derives the project name from the deploy directory's basename (`dev`/`prod`, shared by ALL apps in the /opt/<slug>/<env> layout), and `up -d --remove-orphans` would delete other apps' containers.
